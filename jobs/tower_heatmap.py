...
from pyspark.sql.functions import col, hour, to_timestamp, coalesce
...
def main():
    if len(sys.argv) < 2:
        print("Usage: tower_heatmap.py <run_id> [<input_path>]")
        sys.exit(1)
    ...
    try:
        ...
        # Parse timestamp and extract hour (UTC guaranteed by spark session setting)
        df_with_hour = df.withColumn("hour_of_day", hour(coalesce(
